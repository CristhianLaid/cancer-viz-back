...
export class AuthService {
    ...
    async login(loginAuthDto: LoginAuthDto) {
        ...
        try {
            ...
            if (!auth) throw new UnauthorizedException(`User with email ${email} does not exist`);
            if (!bcryptPlugin.compareSync(password, auth.password)) throw new UnauthorizedException(`Credentials do not match`);
            return {...omit(auth, ['password']), jwt: this.getJwtToken({id: auth.id, username: auth.username})}
        } catch (error: any) {
            this.handlerDbError.logError(error)
        }
    }
